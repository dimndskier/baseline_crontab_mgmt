...
#
class baseline_crontab_mgmt::backout ( $bmenu_path = '/admin' ) {
   cron { 'add_buildmenu':
      ensure  => 'present',
      command => "$bmenu_path/masmenu/buildmenu.sh >>/var/log/buildmenu.log 2>&1",
      user    => root,
      hour    => '2',
      minute  => '0',
   }
...
class baseline_crontab_mgmt::backout::tmpclearadd {
...
class baseline_crontab_mgmt::backout::restartgoferadd (
        $hour_set_gofer = '*/2',
        $min_set_gofer = '0',
){
      cron { 'restart_goferd':
